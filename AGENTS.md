...
**A category × month is a LEDGER, not a cell — and `Plan` is the opposite.** `Plan` carries a unique `{userId, categoryId, month}` and upserts; `Actual` deliberately carries **no** such index, because a category is spent against several times in a month and each spend is its own row with its own note and its own delete. Writes are plain inserts (`repo.createActual()`, `repo.createActuals()` for an import), and the report's `$sum` per cell is what adds them together — many rows and one row read identically downstream. Do not "restore symmetry" by giving Actual the unique index; `tests/indexes.test.ts` asserts a second entry is accepted.

The half that does not live in the schema: **Mongoose never drops an index it stopped declaring.** A database created before this change keeps `userId_1_categoryId_1_month_1`, and the second entry for a cell then fails with a duplicate-key error that has no line of code to blame. The deployed cluster has already had it dropped; any other database needs `db.actuals.dropIndex("userId_1_categoryId_1_month_1")` once, by hand. Never `syncIndexes()` — it drops every index the schema does not declare, including the neighbouring application's. Fresh databases (tests, a new deploy) never had it.

**Import idempotency is per FILE, not per cell.** Since rows append, a re-upload would otherwise stack a second copy of every line. `commitCsv` derives `importBatchId` as a SHA-256 of the CSV text and clears that batch (`repo.deleteActualsByBatch`) inside the same transaction before writing, so importing the same file twice is importing it once. An _edited_ file is a different batch and its unchanged lines are written again — the documented ceiling, marked `ponytail:` in `importCsv.ts`.
