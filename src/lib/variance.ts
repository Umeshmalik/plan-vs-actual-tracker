/**
 * Pure variance math, owning all the edge cases:
 *  - variance = actual - plan (negative = under plan = favorable)
 *  - plan === 0 -> variancePct is null; never NaN/Infinity
 *  - missing actual -> 0 everywhere, row flagged hasActuals:false
 */
...
/**
 * Grouped by month WITHOUT netting: a large overspend and an equally large
 * underspend must not report as on plan. `months` comes from the range, not the
 * rows, so an untouched month is `hasData:false` rather than a silent zero.
 */
